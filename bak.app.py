...
# NB if you set GITHUB_ACCESS_TOKEN, you should also enable basic auth, 
# otherwise you'll have uploads to your wiki repo open to the Internet:
repository = os.getenv('GITHUB_REPOSITORY', '')
branch = os.getenv('GITHUB_BRANCH', 'master')
access_token = os.getenv('GITHUB_ACCESS_TOKEN', '')
print(f"Github repository {repository}, branch {branch}. Access token set: {access_token != ''}")
...
def upload_post():
    ...
    token = access_token or request.form.get('access_token')
    if upload and token:
        ...
        try:
            ...
            repo.update_file(path, "Wiki file update", content, gh_contents.sha, branch=branch)
        except UnknownObjectException:
            print(f"Creating {path}")
            repo.create_file(path, "Wiki file upload", content, branch=branch)
        #repo.delete_file(gh_contents.path, "Wiki file deleted", gh_contents.sha, branch=branch)

        # If adding to Github succeeded then save the file locally
